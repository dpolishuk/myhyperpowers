...
Options:
  ...
  --codex-home <path>   Override Codex home directory (exact .codex path)
...
Examples:
  ...
  $(basename "$0") --codex-home /custom/.codex
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --codex-home)
      ...
      [[ -z "$CODEX_HOME_OVERRIDE" ]] && error "--codex-home requires a value"
      shift 2
      ;;
    --force)
      FORCE="true"
...
if [[ -n "$CODEX_HOME_OVERRIDE" ]]; then
  CODEX_ROOT="$CODEX_HOME_OVERRIDE"
else
  if [[ "$MODE" == "global" ]]; then
    CODEX_ROOT="$HOME/.codex"
  else
    BASE_TARGET="${TARGET:-$PWD}"
    CODEX_ROOT="$BASE_TARGET/.codex"
  fi
fi

TARGET_SKILLS_DIR="$CODEX_ROOT/skills"
VERSION_FILE="$CODEX_ROOT/.hyperpowers-codex-version"
BACKUP_ROOT="$CODEX_ROOT/.hyperpowers-codex-backup"
...
backup_existing() {
  if ! has_installed_wrappers; then
    return
  fi
...
if [[ "$CURRENT_VERSION" == "$INSTALLED_VERSION" && "$FORCE" != "true" ]] && has_installed_wrappers; then
