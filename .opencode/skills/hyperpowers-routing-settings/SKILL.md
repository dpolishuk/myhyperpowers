...
This is the **primary settings-like UX** for Hyperpowers routing on OpenCode.
It provides a **plugin-owned settings workflow** over the shared routing backend.

## Step 1: Show current state

Call `hyperpowers_agent_routing_config` with `action=get` NOW. Render the result as a markdown table:
