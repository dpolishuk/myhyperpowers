...
type OpenCodeConfig = Record<string, unknown> & {
  ...
  hyperpowers?: Record<string, unknown> & {
...
const createRoutingSnapshot = (config: OpenCodeConfig, configPath: string) => ({
  ok: true,
  sourceOfTruth: "opencode.json",
  configPath,
  ...
  routing: {
    ...
    hyperpowers: {
      workflowOverrides: normalizeWorkflowOverrides(config.hyperpowers?.workflowOverrides),
    },
  },
})
...
const persistConfig = async (configPath: string, config: OpenCodeConfig) => {
...
const updateWorkflowAgentModel = (
  ...
  const nextConfig: OpenCodeConfig = { ...config }
  const hyperpowers = asRecord(nextConfig.hyperpowers)
  const workflowOverrides = asRecord(hyperpowers.workflowOverrides)
  const workflowEntry = asRecord(workflowOverrides[workflowName])
  const agentEntry = asRecord(workflowEntry[agentName])

  nextConfig.hyperpowers = {
    ...hyperpowers,
...
const executeRoutingAction = async (rootDir: string, args: RoutingToolArgs) => {
  ...
  if (args.action === "get") {
    ...
    return createRoutingSnapshot(current.config, configPath)
  }
  ...
  const nextConfig = workflowName
    ? updateWorkflowAgentModel(current.config, workflowName, agentName, model)
    : updateGlobalAgentModel(current.config, agentName, model)
  await persistConfig(configPath, nextConfig)

  return {
    ...createRoutingSnapshot(nextConfig, configPath),
    createdConfig: current.createdConfig,
    updatedPath: workflowName
      ? `hyperpowers.workflowOverrides.${workflowName}.${agentName}.model`
  ...
  return {
    tool: {
      hyperpowers_agent_routing_config: tool({
        description:
          "Read or update the shared Hyperpowers agent-model routing map in the project opencode.json file.",
        args: {
