## Description

Brief description of the changes in this PR.

## Related Issues

Fixes #(issue number) or references epic/task IDs.

## Checklist

- [ ] I have read the project `AGENTS.md` and followed the coding style guidelines.
- [ ] I have run `node --test tests/*.test.js` and all tests pass.
- [ ] I have run `node scripts/sync-codex-skills.js --check` and it passes.
- [ ] If I modified skills, agents, or commands, I ran `node scripts/sync-codex-skills.js --write` to regenerate Codex wrappers.
- [ ] I have added or updated tests for any new or changed functionality.
- [ ] I have updated relevant documentation (`docs/`, `README.md`, `AGENTS.md`, etc.) if needed.
- [ ] I have verified that CI will pass with these changes (`npm audit --audit-level=moderate` is clean).

## Host Platform Impact

- [ ] Claude Code
- [ ] OpenCode
- [ ] Gemini CLI
- [ ] Kimi CLI
- [ ] Codex CLI
- [ ] Pi
- [ ] Core framework / All hosts

## Screenshots / Logs

If applicable, add screenshots or log output to help explain your changes.
