...
test("global install copies codex wrappers and writes version file", () => {
  ...
  try {
    ...
    const installedSkill = path.join(tempHome, ".agents", "skills", "codex-skill-brainstorming", "SKILL.md")
    const versionFile = path.join(tempHome, ".agents", ".hyperpowers-codex-version")
...
test("local install supports explicit target directory", () => {
  ...
  try {
    ...
    const installedSkill = path.join(tempTarget, ".agents", "skills", "codex-command-execute-ralph", "SKILL.md")
    const versionFile = path.join(tempTarget, ".agents", ".hyperpowers-codex-version")
...
test("installer backs up existing codex wrappers before overwrite", () => {
  const tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "codex-installer-home-"))
  const preexistingSkill = path.join(tempHome, ".agents", "skills", "codex-skill-brainstorming", "SKILL.md")
  ...
  try {
    ...
    const backupRoot = path.join(tempHome, ".agents", ".hyperpowers-codex-backup")
    const backups = fs
